...
{
  private static final Logger logger = LoggerFactory.getLogger(DataResultSnapshotSerializer.class);
  ...
  private String serializeHelper(Message result, ResultFormatter resultFormatter) throws Exception
  {
    ...
    for(GPOMutable value: gResult.getValues()) {
      Fields fields = ((DataQuerySnapshot) gResult.getQuery()).getFields();
