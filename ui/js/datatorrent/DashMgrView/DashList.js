...
var DashItem = BaseView.extend({
    ...
    events: {
        "click label.dash_id": "catchIfSelected",
        "dblclick label.dash_id": "editDashId",
        "blur input.dash_id": "saveDashId",
        "keydown input.dash_id": "checkForEnter",
        "click i.removeDash": "removeDash"
    },
    
    catchIfSelected: function(evt) {
        evt.preventDefault();
        if (this.model.get('selected')) return;
        
        if (this.__click_timeout__) {
            clearTimeout(this.__click_timeout__);
            return;
        }
        
        this.__click_timeout__ = setTimeout(_.bind(function() {
