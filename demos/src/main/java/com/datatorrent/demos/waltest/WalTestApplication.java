...
@ApplicationAnnotation(name="HDSWalTesting")
public class WalTestApplication implements StreamingApplication
{
  ...
  {
    dag.setAttribute(DAG.APPLICATION_NAME, "HDSWalTesting");

    Generator gen = dag.addOperator("Generator", new Generator());
    HDSOperator hdsOut = dag.addOperator("HDSOut", new HDSOperator());

    TFileImpl hdsFile = new TFileImpl.DefaultTFileImpl();
    hdsOut.setFileStore(hdsFile);
    hdsOut.setMaxWalFileSize(2 * 1024 * 1024);

    dag.setInputPortAttribute(hdsOut.input, PortContext.PARTITION_PARALLEL, true);
