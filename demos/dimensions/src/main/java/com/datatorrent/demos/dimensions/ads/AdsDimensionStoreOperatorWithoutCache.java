 ...
 */
public class AdsDimensionStoreOperatorWithoutCache extends AbstractSinglePortHDHTWriter<AdInfoAggregateEvent> implements RemovalListener<AdInfoAggregateEvent, AdInfoAggregateEvent>
{
  ...
  LoadingCache<AdInfoAggregateEvent, AdInfoAggregateEvent> aggrCache = null;
  ...
  {
    public AdsDimensionStoreOperatorWithoutCache operator;
  ...
  {
    Iterator<Entry<String, TimeSeriesQuery>> it = this.timeSeriesQueries.entrySet().iterator();
    while (it.hasNext()) {
      Entry<String, TimeSeriesQuery> e = it.next();
      ...
      for (HDSQuery query : rangeQuery.points) {
        ...
        AdInfoAggregateEvent ae;
        try {
          ae = aggrCache.get(rangeQuery.prototype);
        ...
        if (query.processed && query.result != null) {
          ae = super.codec.fromKeyValue(query.key, query.result);
          if (ae != null)
            res.data.add(ae);
        }
  ...
  static class TimeSeriesQuery
  {
    ...
    private transient LinkedHashSet<HDHTReader.HDSQuery> points = Sets.newLinkedHashSet();
  ...
  static public class TimeSeriesQueryResult
  {
    ...
    public List<AdInfoAggregateEvent> data;
  }
